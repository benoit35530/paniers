...
case "confajout":
    ...
    if($message != "") {
        ...
        $rep = mysqli_query($GLOBALS["___mysqli_ston"], "insert into $base_produits (id,description,prix,idproducteur,datemodif) values ('','$description','$prix','$idproducteur',now())");
        ...
        ecrire_log_admin("Produit n° $last_id ajouté : $description $prix");
    }
...
case "modif":
    ...
    if(!isset($id)) {
        ...
        $rep = mysqli_query($GLOBALS["___mysqli_ston"], "select id,description,prix,idproducteur from $base_produits where id = '$id'");
        if(mysqli_num_rows($rep) != 0)
        {
            list($id,$description,$prix,$idproducteur) = mysqli_fetch_row($rep);
            echo formulaire_produit("modif",$id,$description,$prix,$idproducteur);
        }
        else
        {
...
case "confmodif":
    ...
    if(!isset($id)) {
        ...
        if(mysqli_num_rows($rep) != 0) {
            ...
            {
                mysqli_query($GLOBALS["___mysqli_ston"], "update $base_produits set description='$description',prix='$prix',idproducteur='$idproducteur',datemodif=now() where id='$id'");
                ecrire_log_admin("Produit n° $id modifié : $description $prix");
    ...
    if(!isset($id)) {
        ...
        if(mysqli_num_rows($rep) != 0) {
            ...
            if(mysqli_num_rows($rep0) == 0) {
                list($id,$description,$prix,$idproducteur) = mysqli_fetch_row($rep);
                echo formulaire_produit("suppr",$id,$description,$prix,$idproducteur);
            }
            else {
...
case "confsuppr":
    ...
    if(!isset($id)) {
        ...
        $rep = mysqli_query($GLOBALS["___mysqli_ston"], "select id,description from $base_produits where id = '$id'");
        if(mysqli_num_rows($rep) != 0) {
            $rep0 = mysqli_query($GLOBALS["___mysqli_ston"], "select * from $base_commandes where idproduit = '$id'");
            if(mysqli_num_rows($rep0) == 0) {
                ...
                ecrire_log_admin("Produit n° $id supprimé : $description");
            }
            else {
                echo afficher_message_erreur("Le produit n° $id ne peut pas être supprimé : il a encore des commandes !!!");
            }
