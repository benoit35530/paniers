...
function afficher_formulaire_bon_commande($idperiode=0,$iddepot=0,$qteproduit=array(),$action="enregistrercde",
    ...
    $champs["taille"][] = "";
    $champs["nomvar"][] = "";
    $champs["valeur"][] = afficher_periode($idperiode);
    $champs["aide"][] = "";
...
function supprimer_bon_commande($id) {
    global $base_bons_cde, $base_commandes, $base_avoirs;
...
function retrouver_quantites_commande($idcommande) {
    global $base_commandes;
...
function retrouver_commande($idclient, $idperiode) {
    global $base_bons_cde;
    $rep = mysqli_query($GLOBALS["___mysqli_ston"], "select id from $base_bons_cde where idclient = '$idclient' and idperiode = '$idperiode'");
    if (mysqli_num_rows($rep) > 0) {
