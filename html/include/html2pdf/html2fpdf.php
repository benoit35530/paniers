...
class HTML2FPDF extends FPDF
{
    ...
    function OpenTag($tag,$attr)
    {
        ...
        switch($tag){
            ...
            case 'TD':
                ...
                if (isset($attr['WIDTH'])) $this->cell[$this->row][$this->col]['w'] = ConvertSize($attr['WIDTH'],isset($this->table['w']) ? $this->table['w'] : $this->pgwidth);
                if (isset($attr['HEIGHT'])) $this->cell[$this->row][$this->col]['h']	= ConvertSize($attr['HEIGHT'],isset($this->table['w']) ? $this->table['w'] : $this->pgwidth);
                ...
                if (isset($attr['CLASS'])) $this->cell[$this->row][$this->col]['class']	= $attr['CLASS'];
        ...
        for($j = 0 ; $j < $nc ; $j++ ) //columns
        {
            ...
            for($i = 0 ; $i < $nr ; $i++ ) //rows
            {
                if (isset($cs[$i][$j]) && $cs[$i][$j])
                {
                    ...
                    if ($minwidth < 0 and (-$minwidth +2) > $wc['miw']) $wc['miw'] = (-$minwidth) +2; //increase minimum width
    ...
    function _tableWrite(&$table){
        ...
        $x0 = $this->x;
        $y0 = $this->y;
        $right = $this->pgwidth + $this->lMargin;
        ...
        for( $i = 0 ; $i < $numrows ; $i++ ) //Rows
        {
            ...
            for( $j = 0 ; $j < $numcols ; $j++ ) //Columns
            {
                if (isset($cells[$i][$j]) && $cells[$i][$j])
                {
                    ...
                    if( isset($cell['class']) or isset($cell['ID']) or isset($cell['STYLE']) )
                    {
                        if (isset($cell['class'])) $properties = $this->CSS[$cell['class']];
                        elseif (isset($cell['id'])) $properties = $this->CSS[$cell['id']];
                        //Read Inline CSS
                        if (isset($cell['style'])) $properties = $this->readInlineCSS($cell['style']);
                        if (is_array($properties))
                        {
                            ...
                            foreach($properties as $k => $v)
                            {
                                switch($k){
                                    case 'WIDTH':
                                        $cell['w']= ConvertSize($v,$this->pgwidth);
                                        break;
                                    case 'HEIGHT':
                                        $cell['h'] = ConvertSize($v,$this->pgwidth);
                                        break;
                                    case 'BORDER':
                                        $cell['border'] = "1";
                                        break;
                                    case 'BACKGROUND': // bgcolor only
                                        $cell['bgcolor'] = "$v";
                                        break;
                                    case 'BACKGROUND-COLOR': // bgcolor only
                                        $cell['bgcolor'] = "$v";
                                        break;
                                    default:
                                        $otherprops[$k]=$v;
                        ...
                        $this->backupcss = $otherprops;
                        if (!empty($otherprops)) $this->setCSS($otherprops); //name found in the CSS array!
                    }
